import algosdk from "algosdk";
import crypto from "crypto";
import { useEffect, useState } from 'react';
import Token from '../models/tokenModel';
import styles from '../styles/Home.module.css';
import Router from 'next/router';
import Swal from 'sweetalert2';

declare var AlgoSigner: any;

interface Token {
  assetId: number, 
  orderId: number,
  transactionId: string,
  claimer: string,
  company: string,
  risk: string,
  claimed: boolean
}

export const Revoke = (props: any) => {
  
  const [noTokens, setNoTokens] = useState(true);
  const [tokenList, setTokenList] = useState<Array<Token>>()
  const [isLoading, setIsLoading] = useState<boolean>(false)

  const refreshPage = () => {
    Router.push('/revoke')
  }

  useEffect(() => {
    const result = props.props.tokens.filter((x: any) => x.revoked !== true);
    if(result.length === 0) {
      setNoTokens(true);
    } else {
      setNoTokens(false);
      setTokenList(result);
    }
  }, [props.props.tokens]);

  const updateToRevoked = async (assetId: Number, claimed: Boolean, revoked: Boolean) => {
    // Stores token in off-chain issuer repository for future reference
    const res = await fetch("/api/token/update", {
      method: "POST",
      headers: {
        "Content-Type": "application/json"
      }, 
      body: JSON.stringify({
        assetId,
        claimed,
        revoked
      })
    })
    const data = await res.json();
    console.log(data);
  }

  const revokeToken = async (props: any, assetId: number, orderId: number, claimer: string, company: string, risk: string, claimed: boolean) => {

    setIsLoading(true);
    // Connect to node
    const algodServer = props.props.ALGOD_SERVER;
    const algoIndexer = props.props.ALGO_INDEXER;
    const token = { 
      "X-API-Key": props.props.ALGOD_TOKEN
    };
    const port = "";
    const algodClient = new algosdk.Algodv2(token, algodServer, port);
    const algoIndexerClient = new algosdk.Indexer(token, algoIndexer, port);
    console.log(algoIndexer);
    console.log(algoIndexerClient);
    let client: string = localStorage.getItem("accountInformation")!;
    const parslAddr = algosdk.mnemonicToSecretKey(props.props.PARSL_MNEMONIC).addr;
    const parslSk = algosdk.mnemonicToSecretKey(props.props.PARSL_MNEMONIC).sk;
    // Get params from network
    let suggestedParams = await algodClient.getTransactionParams().do();
    
    if(claimed) {     
      // 1) Issuer unfreezes asset
      const freezeObj: any = {
        from: parslAddr,
        freezeTarget: client,
        freezeState: false,
        assetIndex: assetId,
        suggestedParams: suggestedParams
      };
      const unfreezeTxn = algosdk.makeAssetFreezeTxnWithSuggestedParamsFromObject(freezeObj);

      // 2) Issuer updates metadata
      let metadataJson = {
        "standard": "ARC5114",
        "issuer": parslAddr,
        "claimer": claimer,
        "status": "revoked",
        "description": "Cannabis Order Verification Certificate",
        "properties": {
          "Company": company,
          "Order": orderId,
          "Risk": risk
        },
        "mime_type": "image/png"
      }
      const hash = crypto.createHash('sha256');
      hash.update(JSON.stringify(metadataJson))
      const metadata = new Uint8Array(hash.digest());
      const updateObj: any = {
        from: parslAddr,
        assetIndex: assetId,
        manager: parslAddr,
        reserve: undefined,
        clawback: parslAddr,
        freeze: parslAddr,
        strictEmptyAddressChecking: false,
        note: AlgoSigner.encoding.stringToByteArray(JSON.stringify(metadataJson)),
        assetMetadataHash: metadata,
        suggestedParams: suggestedParams,
      };
      const updateTxn = algosdk.makeAssetConfigTxnWithSuggestedParamsFromObject(updateObj);

      // 3) Issuer clawbacks token from claimer's account
      const sendObj: any = {
        from: parslAddr,
        to: parslAddr,
        amount: 1,
        assetIndex: assetId,
        revocationTarget: client,
        suggestedParams: suggestedParams
      };
      const clawbackTxn = algosdk.makeAssetTransferTxnWithSuggestedParamsFromObject(sendObj);

      // Atomic Transfer
      algosdk.assignGroupID([unfreezeTxn, updateTxn, clawbackTxn ]);
      let binaryTxs = [unfreezeTxn.toByte(), updateTxn.toByte(), clawbackTxn.toByte()];

      let signedTx0Binary = unfreezeTxn.signTxn(parslSk);
      let signedTx1Binary = updateTxn.signTxn(parslSk);
      let signedTx2Binary = clawbackTxn.signTxn(parslSk);

      let signed = []   
      signed.push(signedTx0Binary);
      signed.push(signedTx1Binary);
      signed.push(signedTx2Binary);

      let tx = (await algodClient.sendRawTransaction(signed).do());
      await algosdk.waitForConfirmation(algodClient, tx.txId, 4);
      const revokingTransaction = await algoIndexerClient.lookupTransactionByID(tx.txId).do();
      let groupId = encodeURIComponent(revokingTransaction.transaction.group);
      await updateToRevoked(assetId, true, true);
      const atomicTransfer = `https://testnet.algoexplorer.io/tx/group/` + groupId;
      Swal.fire(
        'Good job',
        `Your token has been revoked. Check <a style="text-decoration: underline" target="_blank" rel="noopener noreferrer" href=${atomicTransfer}><b>atomic transfer</b></a>`,       
        'success'
      )    
      refreshPage();
    } else {
      // Issuer updates metadata
      let metadataJson = {
        "standard": "ARC5114",
        "issuer": parslAddr,
        "claimer": claimer,
        "status": "revoked",
        "description": "Cannabis Order Verification Certificate",
        "properties": {
          "Company": company,
          "Order": orderId,
          "Risk": risk
        },
        "mime_type": "image/png"
      }
      console.log(metadataJson);
      const hash = crypto.createHash('sha256');
      hash.update(JSON.stringify(metadataJson))
      const metadata = new Uint8Array(hash.digest());
      const updateObj: any = {
        from: parslAddr,
        assetIndex: assetId,
        manager: parslAddr,
        reserve: undefined,
        clawback: parslAddr,
        freeze: parslAddr,
        strictEmptyAddressChecking: false,
        note: AlgoSigner.encoding.stringToByteArray(JSON.stringify(metadataJson)),
        assetMetadataHash: metadata,
        suggestedParams: suggestedParams,
      };
      const updateTxn = algosdk.makeAssetConfigTxnWithSuggestedParamsFromObject(updateObj);
      let signedTx = algosdk.signTransaction(updateTxn, parslSk);
      let signed = []
      signed.push(signedTx.blob)
      let tx = (await algodClient.sendRawTransaction(signed).do());
      await algosdk.waitForConfirmation(algodClient, tx.txId, 4);
      if(tx.txId !== undefined) {
        const transaction = `https://testnet.algoexplorer.io/tx/` + tx.txId;
        Swal.fire(
          'Good job',
          `Your token has been revoked. Check <a style="text-decoration: underline" target="_blank" rel="noopener noreferrer" href=${transaction}><b>transaction</b></a>`,       
          'success'
        )
        await updateToRevoked(assetId, false, true);
        refreshPage();
      }
    }

  }

  const render = (props: any) => {
    if(noTokens) {
      return <div style={{display: 'flex', color: "red", justifyContent:'center', alignItems:'center', height: '10vh'}}><h6>No Tokens have been issued by Parsl.</h6></div>
    } else {
      return (<div>
        <table className={styles.table}>
          <thead>
            <tr>
              <th className={styles.thTitle}>Asset ID</th>
              <th className={styles.thTitle}>Company</th>
              <th className={styles.thTitle}>Order ID</th>
              <th className={styles.thTitle}>Risk</th>
              <th className={styles.thTitle}>Action</th>
            </tr>
          </thead>
          {tokenList?.map(({ assetId, orderId, transactionId, claimer, company, risk, claimed }, i) => (
            <tbody key={i}>
              <tr>
                <th className={styles.th}> {assetId} </th>
                <th className={styles.th}> {company} </th>
                <th className={styles.th}> {orderId} </th>
                <th className={styles.th}> {risk} </th>
                <th className={styles.th}><button className={styles.goBackButton} disabled={isLoading} onClick={() => revokeToken(props, assetId, orderId, claimer, company, risk, claimed)
                .then(() => {setIsLoading(false)})
                .catch(() => {setIsLoading(false)})}>Revoke Token</button></th>
              </tr>
            </tbody>
          ))}
        </table>
        </div>);
    }
  }

  return (
    <div>
      <div style={{display: 'flex',  justifyContent:'center', alignItems:'center', height: '1vh'}}>
        <h1>Revoke SoulBound Token</h1>
      </div>
      <div style={{display: 'flex',  justifyContent:'center', alignItems:'center', height: '10vh'}}>
        <h6>Tokens issued by Parsl:</h6>
      </div>
      <br />
      {render(props)}
    </div>
  );
}